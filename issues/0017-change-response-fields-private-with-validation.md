...
`pub struct Response { pub ... }` で全フィールド公開のため、将来 `trailers` 等を追加すると、構造体リテラル `Response { ... }` を使う全利用者がコンパイル不能になる。全フィールド非公開化により、将来のフィールド追加は非破壊的になる。`#[non_exhaustive]` は外部クレートからの網羅的パターンマッチを禁止し、同一 major バージョン内のフィールド追加が downstream のコンパイルを破壊しないことを型レベルで宣言する。
...
| `src/encoder.rs` | 修正 | 全フィールド直接アクセスをアクセサ経由に書き換え、`encode()` doc 更新、二重バリデーション維持、`validate_response_fields` の reason-phrase 検証で空文字列を absent としてスキップ |
| `src/decoder/response.rs` | 修正 | 構造体リテラル → `from_raw_parts` に書き換え、空 reason-phrase 許容対応 |
| `src/error.rs` | 変更不要 | 必要なバリアントは全て既存。追加不要。 |
| `examples/http11_server/src/main.rs` | 修正 | `build_response` / `build_compressed_response` / `add_connection_headers` の戻り値を `Result<Response, EncodeError>` に変更し、`.header(...)` / `.add_header(...)` 連鎖を `?` で繋ぐ。呼び出し元 (`serve_request`) も `?` 伝播対応 (`EncodeError: std::error::Error` を `Box<dyn Error>` に `From` 変換) |
| `examples/http11_server_io_uring/src/main.rs` | 修正 | 同上 |
| `examples/http11_reverse_proxy/src/main.rs` | 修正 | upstream の任意ヘッダーをループで `add_header` する箇所を `add_header(name, value)?` に変更し、関数シグネチャを `Result<Response, EncodeError>` 化する。`.unwrap()` は使わない (任意入力での panic は CLAUDE.md「サンプルはお手本」原則違反) |
| `examples/http11_client/src/main.rs` | 修正 | 構造体リテラル (`line 216-226, 339-349`) を `Response::with_version(...)?` + `add_header(...)?` で再構築する形に書き換え。受信した raw 値が再検証で失敗した場合は呼び出し元で `?` 伝播。`print_response()` 内のフィールドアクセス (`line 354-356, 360, 373`) を accessor 経由に書き換え |
| `tests/test_encoder.rs` | 修正 | `Response::new()` 等の呼び出しに `.unwrap()` 追加 |
...
| `pbt/tests/prop_decoder/response.rs` | 修正 | `Response::new()` 等に `.unwrap()` 追加 |
...
| `tests/test_decoder.rs:1335-1336` | 修正 | `response.status_code` / `response.body.as_deref()` を accessor 経由に書き換え |
| `tests/test_decode_body.rs:237` | 修正 | `response.body.as_deref()` を `response.body()` に書き換え |
| `pbt/tests/prop_decoder/body.rs` | 修正 | `response.body` のフィールド直接アクセスを `response.body()` に書き換え |
...
| `skills/shiguredo-http11/SKILL.md` | 修正 | Response の主要メソッド一覧 (line 36) を新 API (`new` / `with_version` / `add_header` / `set_header` / `is_body_omitted` 等が `Result` 化または新設) に追従させ、サンプルコードに `.unwrap()` を追加する |
...
全フィールドの `pub` を除去し非公開化する。`#[non_exhaustive]` は同一 major バージョン内のフィールド追加が downstream のコンパイルを破壊しないようにするため、外部クレートからの網羅的パターンマッチを禁止する。
...
`src/decoder/response.rs:790-797` の構造体リテラルはこのコンストラクタに書き換える。
...
本 issue では builder `pub fn body(mut self, body: Vec<u8>) -> Self` (line 191 / 上記参照) と getter `pub fn body(&self) -> Option<&[u8]>` (line 175 / 読み取り専用アクセサ) が **同名** で併存する。Rust の inherent impl は `&self` / `mut self` のシグネチャで共存可能なため言語仕様上は問題ない。

判断: 既存 API (`body(body: Vec<u8>) -> Self`) を破壊的に改名すると、本 issue の影響範囲がさらに広がるため、本 issue では同名併存を採用する。将来 `with_body(self) -> Self` 等への改名を検討する場合は別 issue とする。`add_header` (mutator) / `header` (builder) のような既存の命名規則とは非対称になるが、これは `body` ビルダーが既に確立した名前であるため踏襲する。
...
#### encoder の status-line 出力ロジック確認

`encoder.rs` の status-line 生成箇所 (`encode_response` line 798-804、`encode_response_headers` line 1053-1059) は:
```rust
...
```
となっており、`reason_phrase == ""` の場合に `HTTP/1.1 200 \r\n` (status-code の後に SP 1 個、その後すぐ CRLF) が生成される。これは RFC 9112 §4 lines 690-693 の MUST (separates the status-code from the reason-phrase even when ... absent) を満たす。本 issue で生成ロジックの変更は不要。検証方針で実際に空 reason_phrase の output を確認する。
...
| reason_phrase | `&response.reason_phrase` / `response.reason_phrase.as_bytes()` | `response.reason_phrase()` / `.as_bytes()` |
| version | `&response.version` / `response.version.as_bytes()` | `HttpHead::version(response)` / `.as_bytes()` |
...
2. **異なる検証基準**: 構築時の `is_valid_protocol_version` は `token "/" DIGIT+ "." DIGIT+` (RTSP 拡張含む) を検証するが、encode 時は `is_valid_version_for_encode` (VCHAR のみ) で送信安全なバイトのみを保証する。両者は検証の目的が異なる (構文検証 vs 送信安全検証)
...
/// レスポンスをバイト列にエンコード
///
/// 構文上の不正なフィールド値は構築時バリデーションで弾かれるが、
/// 意味論的な RFC 違反のヘッダー組み合わせ (Content-Length 不一致等) が
/// ある場合はパニックする。エラーハンドリングが必要な場合は `try_encode()` を使用する。
...
- [CHANGE] `Response` の全フィールドを非公開化し、構築時バリデーションを追加する
  ...
  - `status_code()` / `reason_phrase()` / `body()` / `is_body_omitted()` の読み取り専用アクセサを追加する
...
注: 初版で言及した `[FIX] is_valid_reason_phrase が空文字列を許容していたのを修正する` は CHANGES.md エントリとして含めない。理由: `is_valid_reason_phrase` は `pub(crate)` で外部公開されておらず、唯一の呼び出し元 (decoder / encoder) は本 issue 適用後も呼び出し前の空文字列スキップで挙動を維持する。外部利用者から見た挙動変化は発生しないため `[FIX]` は不適切。代わりに `### misc` セクションに以下を追加する:

```
### misc

- [UPDATE] (crate 内部) `is_valid_reason_phrase` を RFC 9112 §4 ABNF `1*(HTAB / SP / VCHAR / obs-text)` に厳密準拠させ、空文字列を非合法と判定する。decoder / encoder の呼び出し側で reason-phrase absent (空文字列) はスキップする方式に統一し、受信・送信ともに RFC 9112 §4 の absent ケースを引き続き許容する
  - @voluntas
```
