...
- `src/decoder/head.rs`:
  - `RequestHead` / `ResponseHead` のフィールドを `pub(crate)` に降格し `#[non_exhaustive]` を付与
  - 公開構築 API `new` / `with_version` / `header` / `add_header` を追加 (全てバリデート付き、`Result<_, EncodeError>`)
  - 公開アクセサ `method()` / `uri()` / `version()` / `headers()` / `status_code()` / `reason_phrase()` / `status_class()` を追加
  - decoder 内部用に `pub(crate) fn from_validated_parts(...)` を新設。debug ビルドでは `debug_assert!` で各フィールドの不変式を検査し、release ビルドでは検査をスキップする (cfg 分岐で実装)
  - `status_class()` の `.expect(...)` はフィールド非公開化により `status_code` が 100..=599 不変式を満たすことが型レベルで保証されるため安全
- `src/decoder/request.rs::decode_headers`:
  - `RequestHead { ... }` 構造体リテラルを `RequestHead::from_validated_parts(...)` 呼び出しに変更
- `src/decoder/response.rs::decode_headers`:
  - `ResponseHead { ... }` 構造体リテラルを `ResponseHead::from_validated_parts(...)` 呼び出しに変更
- `src/decoder/mod.rs` の doc サンプルで `head.method` を `head.method()` に修正
- 利用箇所の書き換え:
  - `tests/test_decoder.rs` / `tests/test_request_target.rs`: フィールド直接アクセスを `head.method()` 等のアクセサ呼び出しに変更
  - `pbt/tests/prop_request.rs` / `prop_response.rs` / `prop_decoder/*.rs`: 同上
  - `pbt/tests/prop_decoder/head.rs`: 17 個の `ResponseHead { ... }` 構造体リテラルを、新設したテスト用ヘルパー `make_response_head(version, status_code, reason_phrase, headers)` 経由の構築に書き換え (内部で `with_version` + `add_header` のループ)。バリデーション失敗は `expect` で panic させ、テストデータが valid 範囲に収まっている前提を明示
  - `examples/http11_reverse_proxy/src/main.rs`: `&req_head.headers.iter()` / `&req_head.headers` / `&head.version()` 等をアクセサ呼び出しに書き換え
  - `examples/http11_client/src/transport.rs`: 同上
- `prop_assert_eq!` の比較対象が `&String` から `&str` に変わったことによる `str == &str` エラーは、生成器の型 (`String` か `&'static str`) に応じて `.as_str()` を付けるか `&` を外す形で対応
