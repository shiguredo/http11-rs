...
- `src/validate.rs` に以下の `pub(crate)` 関数を追加:
  - `is_qdtext_byte(b: u8) -> bool`: HTAB / SP / 0x21 / 0x23-0x5B / 0x5D-0x7E / 0x80-0xFF
  - `is_quoted_pair_byte(b: u8) -> bool`: HTAB / 0x20-0x7E / 0x80-0xFF (NUL / CR / LF / 他 CTL を除く)
- `src/auth.rs::parse_auth_params` の quoted-string 解析でバイト単位 (`b as char` ではなく) に `is_qdtext_byte` / `is_quoted_pair_byte` を適用し、CR / LF / NUL を含む quoted-pair / qdtext を `AuthError::InvalidParameter` で reject
- `src/content_disposition.rs::parse_quoted_string` を `chars()` ベースから `bytes()` ベースに書き換え、`is_qdtext_byte` / `is_quoted_pair_byte` で文字集合検証
- `src/cache.rs::parse` の `value.trim().trim_matches('"')` を以下に変更:
  - 値が両端 DQUOTE で囲まれている場合のみ内側を取り出す
  - 片端 DQUOTE のみは `CacheError::InvalidFormat` で reject
- テスト追加:
  - `tests/test_content_disposition.rs::test_content_disposition_quoted_pair_rejects_crlf`: `\<CR>` / `\<LF>` / `\<NUL>` を含む quoted-pair の reject 確認
  - `tests/test_content_disposition.rs::test_content_disposition_qdtext_rejects_crlf`: 生 CR / LF を含む qdtext の reject 確認
  - `tests/test_auth.rs::test_auth_quoted_pair_rejects_crlf` / `test_auth_qdtext_rejects_crlf`: 同等の auth.rs 経路
  - `tests/test_cache.rs::test_cache_control_rejects_partial_open_quote` / `..._partial_close_quote` / `..._accepts_both_sides_quoted` / `..._accepts_unquoted`: cache.rs の対称検査
- `CHANGES.md` の `## develop` に `[FIX]` 2 件を追加した
