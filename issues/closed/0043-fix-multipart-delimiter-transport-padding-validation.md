...
- `src/multipart.rs::next_part` の `Initial` ブランチの `--<boundary>` 直後判定を以下に書き換えた:
  1. `after_delim` 位置から SP / HTAB を 0 個以上スキップする (transport-padding 寛容受理)
  2. スキップ後 2 バイトが `\r\n` → `InPart` に遷移、`--` → `Finished` に遷移、それ以外 → `MultipartError::InvalidPart`
  3. transport-padding 途中で buffer が尽きた場合は `Incomplete` を返し、`boundary_scan_offset` を更新して次回 feed 時に再開する
- 死コードだった旧実装末尾の `starts_with(b"\r\n")` スキップを削除した
- `AfterInnerDelimiter` ブランチで返していた `InvalidBoundary` を `InvalidPart` に統一した (`InvalidBoundary` は boundary 文字列自体の構文不正専用)
- `tests/test_multipart.rs` に dash-boundary 直後の不正バイト reject テスト (CR / `-` / NUL / VCHAR / 数字 / CTL / 非 ASCII の 7 ケース)、transport-padding 寛容受理テスト (SP / HTAB)、transport-padding 途中の Incomplete テストを追加した
- `pbt/tests/prop_multipart.rs` に `prop_multipart_dash_boundary_invalid_byte_is_rejected` を追加した
- `CHANGES.md` の `## develop` に `[FIX]` エントリを追加した

採用案: 案 A (`MultipartError::InvalidPart` を流用)。新規バリアントは追加していない。
