...
- `src/decoder/request.rs::determine_body_kind` の TE 禁止ガードを `version == "HTTP/1.0"` から `version != "HTTP/1.1"` の否定条件に変更した
- `src/decoder/response.rs::determine_body_kind` 同じ TE 禁止ガードを同条件に変更した
- エラーメッセージを「`Transfer-Encoding is not defined in HTTP/1.0`」から「`Transfer-Encoding is only defined for HTTP/1.1`」に揃えた
- 両ファイルのコードコメントに RFC 9112 Section 6.1 + RFC 2326 Section 5 + 将来変更余地の根拠を明記した
- `tests/test_decode_body.rs` に `HTTP/0.9` / `HTTP/2.0` / `HTTP/3.0` / `RTSP/1.0` / `RTSP/2.0` / `FOO/1.0` の reject テストを request / response 両方で追加した (計 13 ケース、case 違いの補助テスト含む)
- 既存の `http10_with_transfer_encoding_should_fail` テストを新エラーメッセージに合わせて更新した
- `pbt/tests/prop_decoder/request.rs` / `prop_decoder/response.rs` に `prop_request_te_rejected_for_non_http11` / `prop_request_te_accepted_for_http11` / `prop_response_te_rejected_for_non_http11` / `prop_response_te_accepted_for_http11` を追加した
