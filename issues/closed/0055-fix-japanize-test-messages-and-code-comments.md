...
- PBT (`pbt/tests/prop_auth.rs`, `prop_encoder.rs`, `prop_decoder/*.rs`, `prop_range.rs`, `prop_request.rs`, `prop_response.rs`, `prop_accept.rs`, `prop_content_type.rs`) の英語 `prop_assert!` メッセージを日本語化した (約 64 件)
- 単体テスト (`tests/test_decoder.rs`, `test_decode_body.rs`, `test_request.rs`, `test_host.rs`, `test_request_target.rs`, `test_status_code.rs`) の英語 `assert!` / `assert_eq!` / `panic!` メッセージを日本語化した (約 35 件)
- examples のテスト (`examples/http11_client/tests/*` / `examples/http11_server/tests/*`) の `.expect(...)` / `panic!(...)` / `assert!(...)` メッセージを日本語化した (約 60 件)
- `src/encoder.rs` / `src/decoder/request.rs` / `src/decoder/response.rs` / `examples/http11_client/src/main.rs` の英語コメントを日本語化または削除した (約 14 件)
- `pbt/tests/prop_request.rs` / `prop_content_type.rs` / `prop_accept.rs` の廃止 RFC 7230 参照を RFC 9110 Section 5.6.2 に置き換えた
- ログメッセージ (`tracing::*`) / エラーメッセージ (`Error::*` / `#[error(...)]`) / `///` doc コメントは変更していない
- `CHANGES.md` の `## develop` セクション `### misc` に `[FIX]` エントリを追加した
