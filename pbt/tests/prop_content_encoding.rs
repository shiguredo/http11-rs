...
#[test]
fn content_encoding_error_display() {
    let errors = [
...
#[test]
fn content_encoding_error_is_error_trait() {
...
proptest! {
    #[test]
    fn content_encoding_standard_roundtrip(enc in standard_encoding()) {
...
proptest! {
    #[test]
    fn content_encoding_case_insensitive(enc in mixed_case_encoding()) {
        let ce = ContentEncoding::parse(&enc).unwrap();
...
proptest! {
    #[test]
    fn content_encoding_custom_roundtrip(enc in custom_encoding()) {
        let ce = ContentEncoding::parse(&enc).unwrap();
...
proptest! {
    #[test]
    fn content_encoding_multiple(encodings in proptest::collection::vec(standard_encoding(), 1..5)) {
...
proptest! {
    #[test]
    fn content_encoding_has_gzip(prefix in proptest::collection::vec(standard_encoding(), 0..3)) {
...
proptest! {
    #[test]
    fn content_encoding_has_deflate(prefix in proptest::collection::vec(standard_encoding(), 0..3)) {
        let mut all = prefix.clone();
        all.push("deflate");
...
proptest! {
    #[test]
    fn content_encoding_has_compress(prefix in proptest::collection::vec(standard_encoding(), 0..3)) {
        let mut all = prefix.clone();
        all.push("compress");
...
proptest! {
    #[test]
    fn content_encoding_has_identity(prefix in proptest::collection::vec(standard_encoding(), 0..3)) {
        let mut all = prefix.clone();
        all.push("identity");
...
#[test]
fn content_encoding_parse_errors() {
    // 空
    assert!(matches!(
...
// 末尾のカンマは許可される
#[test]
fn content_encoding_trailing_comma() {
...
#[test]
fn content_encoding_empty_tokens() {
    let ce = ContentEncoding::parse("gzip,, deflate").unwrap();
    assert_eq!(ce.encodings().len(), 2);
    assert!(ce.has_gzip());
...
proptest! {
    #[test]
    fn content_encoding_clone_eq(encodings in proptest::collection::vec(standard_encoding(), 1..5)) {
        let input = encodings.join(", ");
        let ce = ContentEncoding::parse(&input).unwrap();
        let cloned = ce.clone();
...
proptest! {
    #[test]
    fn content_encoding_parse_no_panic(s in "[ -~]{0,64}") {
