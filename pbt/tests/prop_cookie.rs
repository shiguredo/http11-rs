...
proptest! {
    ...
    fn prop_set_cookie_roundtrip(name in "[a-zA-Z][a-zA-Z0-9_-]{0,15}", value in "[a-zA-Z0-9_-]{0,32}") {
        let cookie = SetCookie::new(&name, &value).expect("Cookie のパースは成功するはず (実装バグ)");
        let displayed = cookie.to_string();
        let reparsed = SetCookie::parse(&displayed, 2026).expect("Cookie のパースは成功するはず (実装バグ)");
...
proptest! {
    ...
    fn prop_set_cookie_with_domain(name in "[a-zA-Z][a-zA-Z0-9]{0,7}", value in "[a-zA-Z0-9]{0,16}", domain in "[a-z]{1,8}\\.[a-z]{2,4}") {
        let cookie = SetCookie::new(&name, &value).expect("Cookie のパースは成功するはず (実装バグ)").with_domain(&domain);
