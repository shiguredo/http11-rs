...
proptest! {
    ...
    fn prop_invalid_utf8_request_line_error(
        ...
        let mut decoder = RequestDecoder::new();
        decoder.feed(&data).expect("リクエストのデコードは成功するはず (実装バグ)");
        prop_assert!(decoder.decode_headers().is_err());
    }
}
...
proptest! {
    ...
    fn prop_incomplete_request_line(
        ...
        let mut decoder = RequestDecoder::new();
        decoder.feed(data.as_bytes()).expect("リクエストのデコードは成功するはず (実装バグ)");
        prop_assert!(decoder.decode_headers().expect("リクエストのデコードは成功するはず (実装バグ)").is_none());
    }
}
...
proptest! {
    ...
    fn prop_incomplete_body(
        ...
        let mut decoder = RequestDecoder::new();
        decoder.feed(data.as_bytes()).expect("リクエストのデコードは成功するはず (実装バグ)");
        let (_, body_kind) = decoder.decode_headers().expect("結果は存在するはず (実装バグ)").expect("リクエストのデコードは成功するはず (実装バグ)");
        prop_assert_eq!(body_kind, BodyKind::ContentLength(body_length as u64));
        let peeked = decoder.peek_body().expect("リクエストのデコードは成功するはず (実装バグ)");
...
proptest! {
    ...
    fn prop_request_decoded_header_memory_bounded(
        ...
        let data = format!("GET / HTTP/1.1\r\nHost: localhost\r\n{}\r\n\r\n", headers);
        decoder.feed(data.as_bytes()).expect("リクエストのデコードは成功するはず (実装バグ)");
        let (head, _) = decoder.decode_headers().expect("結果は存在するはず (実装バグ)").expect("リクエストのデコードは成功するはず (実装バグ)");
...
proptest! {
    ...
    fn prop_multiple_requests_same_decoder(
        ...
        for i in 0..count {
            let mut request = Request::new(methods[i].clone(), &uris[i]).expect("リクエストのデコードは成功するはず (実装バグ)");
            request.add_header(HeaderName::from_static(b"Host"), "localhost").expect("リクエストのデコードは成功するはず (実装バグ)");
            let encoded = request.encode().expect("リクエストのデコードは成功するはず (実装バグ)");
            decoder.feed(&encoded).expect("リクエストのデコードは成功するはず (実装バグ)");
            let decoded = decoder.decode().expect("結果は存在するはず (実装バグ)").expect("リクエストのデコードは成功するはず (実装バグ)");
...
proptest! {
    ...
    fn prop_decoder_reuse_after_error(
        ...
        decoder.reset();
        // リセット後は正常に動作
        let mut request = Request::new(valid_method.clone(), &valid_uri).expect("リクエストのデコードは成功するはず (実装バグ)");
        request.add_header(HeaderName::from_static(b"Host"), "localhost").expect("リクエストのデコードは成功するはず (実装バグ)");
        decoder.feed(&request.encode().expect("リクエストのデコードは成功するはず (実装バグ)")).expect("リクエストのデコードは成功するはず (実装バグ)");
        let decoded = decoder.decode().expect("結果は存在するはず (実装バグ)").expect("リクエストのデコードは成功するはず (実装バグ)");
...
proptest! {
    ...
    fn prop_streaming_decode_request(
        ...
        decoder.feed(data.as_bytes()).expect("リクエストのデコードは成功するはず (実装バグ)");
        let (head, body_kind) = decoder.decode_headers().expect("結果は存在するはず (実装バグ)").expect("リクエストのデコードは成功するはず (実装バグ)");
...
proptest! {
    ...
    fn prop_streaming_decode_request_with_body(
        ...
        while let Some(data) = decoder.peek_body() {
            ...
            let len = data.len();
            if let BodyProgress::Complete { .. } = decoder.consume_body(len).expect("リクエストのデコードは成功するはず (実装バグ)") {
                break;
            }
        }
...
proptest! {
    ...
    fn prop_decode_headers_twice_returns_none(
        ...
        let mut decoder = RequestDecoder::new();
        decoder.feed(data.as_bytes()).expect("リクエストのデコードは成功するはず (実装バグ)");
        let _ = decoder.decode_headers().expect("結果は存在するはず (実装バグ)").expect("リクエストのデコードは成功するはず (実装バグ)");
...
proptest! {
    ...
    fn prop_decode_multiple_requests_keep_alive(
        ...
        for i in 0..count {
            let mut request = Request::new(methods[i].clone(), &uris[i]).expect("リクエストのデコードは成功するはず (実装バグ)");
            request.add_header(HeaderName::from_static(b"Host"), "localhost").expect("リクエストのデコードは成功するはず (実装バグ)");
            all_data.extend(request.encode().expect("リクエストのデコードは成功するはず (実装バグ)"));
        }
        decoder.feed(&all_data).expect("リクエストのデコードは成功するはず (実装バグ)");

        // decode() を連続して呼ぶ（reset() なし）
        for i in 0..count {
            let request = decoder.decode().expect("結果は存在するはず (実装バグ)").expect("リクエストのデコードは成功するはず (実装バグ)");
...
proptest! {
    ...
    fn prop_decode_multiple_requests_with_body_keep_alive(
        ...
        for body_data in &bodies {
            let mut request = Request::new(Method::POST, "/").expect("リクエストのデコードは成功するはず (実装バグ)");
            request.add_header(HeaderName::from_static(b"Host"), "localhost").expect("リクエストのデコードは成功するはず (実装バグ)");
...
proptest! {
    ...
    fn prop_request_decode_mixed_api_error(
        ...
        let mut decoder = RequestDecoder::new();
        decoder.feed(&full).expect("リクエストのデコードは成功するはず (実装バグ)");
...
proptest! {
    ...
    fn prop_request_asterisk_form(
        ...
        let mut decoder = RequestDecoder::new();
        decoder.feed(data.as_bytes()).expect("リクエストのデコードは成功するはず (実装バグ)");
        let result = decoder.decode_headers().expect("リクエストのデコードは成功するはず (実装バグ)");
    ...
    fn prop_feed_mut_buf_equivalence(
        ...
        let by_feed = {
            ...
            for &size in &chunk_sizes {
                ...
                decoder.feed(&full[offset..end]).expect("リクエストのデコードは成功するはず (実装バグ)");
                offset = end;
            }
            if offset < full.len() {
                decoder.feed(&full[offset..]).expect("リクエストのデコードは成功するはず (実装バグ)");
            }
            decoder.decode().expect("リクエストのデコードは成功するはず (実装バグ)")
        };
        ...
        let by_mut_buf = {
            ...
            for &size in &chunk_sizes {
                ...
                let end = (offset + size).min(full.len());
                let len = end - offset;
                let dst = decoder.mut_buf(len).expect("リクエストのデコードは成功するはず (実装バグ)");
...
proptest! {
    ...
    fn prop_mut_buf_returns_exact_length(len in 0usize..4096) {
        let mut decoder = RequestDecoder::new();
        let buf = decoder.mut_buf(len).expect("リクエストのデコードは成功するはず (実装バグ)");
...
proptest! {
    ...
    fn prop_advance_buf_grows_remaining(
        ...
        let mut decoder = RequestDecoder::new();
        if !prefix.is_empty() {
            decoder.feed(&prefix).expect("リクエストのデコードは成功するはず (実装バグ)");
        }
        let before = decoder.remaining().len();
        let buf = decoder.mut_buf(write_len).expect("リクエストのデコードは成功するはず (実装バグ)");
...
proptest! {
    ...
    fn prop_advance_zero_is_identity(
        ...
        let _ = decoder.mut_buf(write_len).expect("リクエストのデコードは成功するはず (実装バグ)");
        decoder.advance_buf(0);
