...
impl BodyDecoder {
        ...
        match phase {
            DecodePhase::BodyContentLength { remaining } => {
                let len_u64 = u64::try_from(len)
                    .map_err(|_| Error::InvalidData("consume_body: len exceeds u64".to_string()))?;
    ...
    fn process_chunked_size(
        ...
        if let Some(pos) = find_line(buf) {
            ...
            let line_bytes = buf
                .get(..pos)
                .ok_or_else(|| Error::InvalidData("invalid chunk size line bounds".to_string()))?;
...
fn validate_authority_chars(authority: &str) -> Result<(), Error> {
    ...
    while i < bytes.len() {
        let b = *bytes
            .get(i)
            .ok_or_else(|| Error::InvalidData("invalid authority: bounds".to_string()))?;
        if b == b'%' {
            if i + 2 >= bytes.len()
...
fn validate_path_chars(path: &str) -> Result<(), Error> {
    ...
    while i < bytes.len() {
        let b = *bytes
            .get(i)
            .ok_or_else(|| Error::InvalidData("invalid path: bounds".to_string()))?;
        if b == b'%' {
            // パーセントエンコーディング検証
            if i + 2 >= bytes.len() {
                return Err(Error::InvalidData(
...
fn validate_query_chars(query: &str) -> Result<(), Error> {
    ...
    while i < bytes.len() {
        let b = *bytes
            .get(i)
            .ok_or_else(|| Error::InvalidData("invalid query: bounds".to_string()))?;
        if b == b'%' {
            // パーセントエンコーディング検証
            if i + 2 >= bytes.len() {
                return Err(Error::InvalidData(
