...
//! - Transfer-Encoding token の RFC 9110 token 規則検証
//! - chunk-ext の ABNF 検証 (chunk-ext-name / chunk-ext-val / quoted-string)
//!
//! ## なぜ PBT (Property-Based Testing) ではテストできないのか
//!
//! ここに集められたテストは「不完全なデータが与えられた場合のデコーダーの状態」や
//! 「意図的なエラーパス」を検証する。
//! - PBT は「有効な HTTP メッセージ」を生成して正しさ (ラウンドトリップ等) を検証する
//! - 接続切断シナリオや特定の `Err` パスは PBT の生成器では自然に出ない
//! - fuzz はパニック耐性を検証するが、エラーメッセージ内容や状態遷移は検証しない
//!
//! 単体テストはこの空白を埋めるために用意している。
...
fn incomplete_content_length_body() {
    ...
    decoder
        .feed(&[0u8; 50])
        .expect("ボディのデコードは成功するはず (実装バグ)"); // 100 バイト中 50 バイトのみ
...
fn incomplete_chunked_body() {
    ...
    decoder
        .feed(b"5\r\nhello\r\n")
        .expect("ボディのデコードは成功するはず (実装バグ)"); // 終端チャンク "0\r\n\r\n" がない
...
fn close_delimited_decode_with_mark_eof() {
    ...
    let response = decoder
        ...
        .expect("結果は存在するはず (実装バグ)")
        .expect("ボディのデコードは成功するはず (実装バグ)");
    assert_eq!(response.body_bytes(), Some(&b"body data"[..]));
}
